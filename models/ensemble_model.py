...
class AttentionPool(nn.Module):
    ...
    def forward(self, x):
        # x: (batch, seq=1, dim) or (batch, seq, dim)
        if x.dim() == 2:
            x = x.unsqueeze(1)
...
class EnsembleModel(nn.Module):
    ...
    def __init__(self, input_dim: int, gbdt_dim: int = 0, hidden_dims: List[int] = None, dropout: float = 0.2):
        ...
        # Attention pooling (embed dim must match sequence feature dim)
    ...
    def forward(self, x, gbdt_feats: torch.Tensor = None):
        ...
        # LSTM expects seq dimension; we create seq len=1
        seq = proj.unsqueeze(1)
        lstm_out, _ = self.lstm(seq)
        lstm_feat = lstm_out.squeeze(1)

        # Attention pooling over a concatenation of res and proj
        attn_input = torch.cat([proj.unsqueeze(1), res.unsqueeze(1)], dim=1)  # (batch, 2, dim)
